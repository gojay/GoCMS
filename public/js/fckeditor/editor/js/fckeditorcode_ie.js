			...
			if (FCKConfig.FullPage) {
				...
				if (FCKBrowserInfo.IsIE)
					...
					C = '<link href="'
							...
							+ 'css/fck_showtableborders_gecko.css" rel="stylesheet" type="text/css" _fcktemp="true" />';
				C += '<link href="'
						+ FCKConfig.FullBasePath
...
FCK.CreateLink = function(A) {
	...
	if (A.length > 0) {
		if (FCKSelection.GetType() == 'Control') {
			var C = this.EditorDocument.createElement('A');
			C.href = A;
			var D = FCKSelection.GetSelectedElement();
		...
		;
		var E = 'javascript:void(0);/*' + (new Date().getTime()) + '*/';
		FCK.ExecuteNamedCommand('CreateLink', E);
		var F = this.EditorDocument.links;
		for (i = 0; i < F.length; i++) {
			var C = F[i];
			if (C.getAttribute('href', 2) == E) {
				var H = C.innerHTML;
				C.href = A;
				C.innerHTML = H;
				var I = C.lastChild;
...
FCKXHtml._AppendChildNodes = function(A, B, C) {
	...
	if (A.childNodes.length == 0) {
		...
		;
		var E = A.nodeName;
		if (FCKListsLib.InlineChildReqElements[E])
			return null;
	...
	for ( var n = 0; n < E.length; n++) {
		...
		if (F.specified) {
			...
			if (G.StartsWith('_fck'))
				...
				H = B.style.cssText.replace(FCKRegexLib.StyleProperties,
...
FCKXHtml.TagProcessors['area'] = function(A, B) {
	...
	if (!A.attributes.getNamedItem('shape')) {
		...
		if (D && D.length > 0)
			FCKXHtml._AppendAttribute(A, 'shape', D);
	}
	;
	return A;
};
...
FCKEditingArea.prototype.Start = function(A, B) {
	...
	if (this.Mode == 0) {
		...
		if (FCKBrowserInfo.IsIE)
			...
			if (FCKBrowserInfo.IsGecko)
				A = A.replace(/(<body[^>]*>)\s*(<\/body>)/i, '$1' + GECKO_BOGUS
						+ '$2');
...
FCKDomRange.prototype.Select = function() {
	if (this._Range) {
		...
		if (!A)
			var C = this._GetRangeMarkerTag(false);
...
FCKDomRange.prototype._GetSelectionMarkerTag = function(A) {
	...
	B.collapse(A === true);
	var C = 'fck_dom_range_temp_' + (new Date()).valueOf() + '_'
			+ Math.floor(Math.random() * 1000);
...
FCKFlashProcessor.RefreshView = function(A, B) {
	if (B.getAttribute('width') > 0)
		A.style.width = FCKTools
				.ConvertHtmlSizeToStyle(B.getAttribute('width'));
	if (B.getAttribute('height') > 0)
		A.style.height = FCKTools.ConvertHtmlSizeToStyle(B
				.getAttribute('height'));
};
	...
	if (C && C.tagName.Equals('TD', 'TH'))
		...
		if (C) {
			for ( var i = 0; i < C.cells.length; i++) {
				...
				if (B.inRange(D)
						|| (B.compareEndPoints('StartToStart', D) >= 0 && B
								.compareEndPoints('StartToEnd', D) <= 0)
						|| (B.compareEndPoints('EndToStart', D) >= 0 && B
								.compareEndPoints('EndToEnd', D) <= 0)) {
...
var FCKPanel = function(A) {
	...
	if (FCKBrowserInfo.IsIE) {
		...
		var E = '';
		if (FCKBrowserInfo.IsSafari)
			E = '<base href="' + window.document.location + '">';
		B.open();
		B
				.write('<html><head>'
						+ E
...
function FCKToolbarSet_Create(A) {
	...
	default:
		...
		G.open();
		G
				.write('<html><head><script type="text/javascript"> window.onload = window.onresize = function() { window.frameElement.height = document.body.scrollHeight ; } </script></head><body style="overflow: hidden">'
...
FCKDialog.Show = function(A, B, C, D, E, F, G) {
	if (!F)
		F = window;
	var H = 'help:no;scroll:no;status:no;resizable:' + (G ? 'yes' : 'no')
			+ ';dialogWidth:' + D + 'px;dialogHeight:' + E + 'px';
	FCKFocusManager.Lock();
	var I = 'B';
	try {
